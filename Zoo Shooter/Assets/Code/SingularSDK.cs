...
public class SingularSDK : MonoBehaviour {
    ...
    private const string UNITY_VERSION = "1.3.0";
    ...
    public static void InitializeSingularSDK() {
		...
		initSDK(instance.SingularAPIKey, instance.SingularAPISecret, facebookAppId, 
			openUri, enableDeferredDeepLinks, instance.ddlTimeoutSec, instance.sessionTimeoutSec, enableLogging, customUserId, imei);
#endif
	...
	private static void initSDK (string APIkey, string secret, string facebookAppId, 
						 string openUri, bool useDeepLinks, long ddlTimeoutSec, long sessionTimeoutSec, bool enableLogging, string customUserId, string imei) {
        ...
        jniSingularUnityBridge.CallStatic("init", APIkey, secret, facebookAppId,
                            openUri, useDeepLinks, ddlTimeoutSec, sessionTimeoutSec, enableLogging, instance.shortlinkResolveTimeout, customUserId, imei);
    ...
    private static extern bool StartSingularSession_(string key, string secret, int shortlinkResolveTimeout);
    ...
    public static bool StartSingularSession(string key, string secret) {
        if (!Application.isEditor) {
            ...
            return StartSingularSession_(key, secret, (Int32)instance.shortlinkResolveTimeout);
#endif
        }
...
#if UNITY_ANDROID
    private static string ExtractAndroidReceiptSignature(string receipt) {         var values = JsonConvert.DeserializeObject<Dictionary<string, string>>(receipt);
