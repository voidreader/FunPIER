...
{
    public static class AN_ResolveManager 
    {
        private const string PLAY_SERVICES_RESOLVER = "Google.JarResolver";
        private const string PLAY_SERVICES_RESOLVER_TYPE = "GooglePlayServices.PlayServicesResolver, Google.JarResolver";
        private const string PLAY_SERVICES_FORCE_RESOLVER_METHOD = "MenuForceResolve";
        ...
        {
            bool detected = IsPathEqualsSDKName(assetPath, PLAY_SERVICES_RESOLVER);
            if (detected) 
            {
                UpdateJarResolverState(true, GetJarResolverVersion(assetPath, PLAY_SERVICES_RESOLVER));
            }
            return detected;
        }
        ...
        private static void CallJarForceResolve()
        {
            try
            {
                var type = Type.GetType(PLAY_SERVICES_RESOLVER_TYPE, false, true);
			    var method = type.GetMethod(PLAY_SERVICES_FORCE_RESOLVER_METHOD);
                if(method != null)
                {
