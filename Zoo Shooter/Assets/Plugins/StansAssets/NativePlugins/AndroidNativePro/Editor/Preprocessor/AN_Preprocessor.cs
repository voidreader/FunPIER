...
namespace SA.Android
{
...
#endif
    {
        ...
        {
            bool plugingVersionUpdated = AN_Settings.UpdateVersion(AN_Settings.FormattedVersion) && !SA_PluginTools.IsDevelopmentMode;
            Refresh();
            var requirements = new AN_AndroidBuildRequirements();
            if (plugingVersionUpdated) 
            {
        ...
        private static void ResolveProperties(AMM_BaseTemplate template, List<AMM_PropertyTemplate> requirementsProperiesList) 
        {
            ...
            foreach (var property in requirementsProperiesList) 
            {
