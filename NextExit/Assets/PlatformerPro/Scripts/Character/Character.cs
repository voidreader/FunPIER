...
namespace PlatformerPro
{
	...
	public class Character : MonoBehaviour, IMob
	{
		...
		/// </summary>
		[Range (0.1f, 2.0f)]
		public float minFastFallPassthroughTime = DefaultMinFastFallPassthroughTime;
		...
		/// </summary>
		protected ExtraDamageInfoEventArgs damageEventArgs;
		...
		/// </summary>
		public const float DefaultPassthroughLeeway = -0.1f;

		/// <summary>
		/// Deafult min ast fall time leeway.
		/// </summary>
		public const float DefaultMinFastFallPassthroughTime = 0.5f;
		...
		#region reference to character component properties

		/// <summary>
		/// Gets CharacterHealth for this Character. Can be null if Character does not have a CharacterHealth.
		/// </summary>
		virtual public CharacterHealth CharacterHealth
		{
			get;
			protected set;
		}
		...
		/// </summary>
		virtual public ItemManager ItemManager
		{
			get;
			protected set;
		}
		...
		/// </summary>
		virtual public PowerUpManager PowerUpManager
		{
			get;
			protected set;
		}
		...
		/// </summary>
		virtual public Inventory Inventory
		{
			get;
			protected set;
		}
		...
		/// </summary>
		virtual public EquipmentManager EquipmentManager
		{
			get;
			protected set;
		}
		...
		virtual public float DefaultGravity
		{
			get
			{
				if (gravity != null) return gravity.Value;
				return -9.8f;
			}
		}
		...
		virtual public Platform ParentPlatform
		{
			...
			set
			{
				if (value == null)
				{
					...
					// In case we get called before Init() (i.e. during respawn).
					if (myTransform != null) myTransform.parent = null;
		...
		/// </summary>
		virtual public int GroundedFootCount 
		{
			get; set;
		}
        ...
        /// </summary>
        public event System.EventHandler <AttackEventArgs> AttackFinished;

		/// <summary>
		/// Event for attack connecting.
		/// </summary>
		public event System.EventHandler <ExtraDamageInfoEventArgs> AttackCausedDamage;
		...
		/// </summary>
		virtual protected void OnAttackFinished(string name)
		{
			if (AttackFinished != null)
			{
				if (activeAttack != -1)
				{
					attackEventArgs.UpdateAttackStartedArgs(name);
					AttackFinished(this, attackEventArgs);
				}
			}
		}
		...
		virtual protected void OnAttackCausedDamage(IMob enemy, DamageInfo info)
		{
			if (AttackCausedDamage != null)
			{
				damageEventArgs.UpdateDamageInfoEventArgs(info, AttackName);
				AttackCausedDamage(this, damageEventArgs);
			}
		}
		...
		void Awake()
		{
			...
			for (int i = 0; i < feetColliders.Length; i++)
			{
				...
				if (basic.Transform == null) basic.Transform = this.transform;
			...
			for (int i = 0; i < headColliders.Length; i++)
			{
				...
				// TODO Trying some lookahead here, move to a variable if it works out, probably need to adjust some of the colldier checks for ceiling movement to handle this
				headColliders[i].LookAhead = sideLookAhead; 
		...
		void Update()
		{
			if (enabled && !TimeManager.Instance.Paused)
			{
				...
				if (currentLadder == null) UpdateCurrentWall();
		...
		virtual public void DoBaseCollisionsForRaycastType(RaycastType type)
		{
			for (int i = 0; i < Colliders.Length; i++)
			{
				if ((Colliders[i].RaycastType & type) == Colliders[i].RaycastType) 
				{
					CurrentCollisions[i] = Colliders[i].GetRaycastHits();
				}
			}
			UpdateClosestColliders (type);
		}
		...
		virtual public Vector2 ApplyBaseCollisionsForRaycastType(RaycastType type)
		{
			...
			Vector2 delta = baseMovement.DoCollisions(type, geometryLayerMask, passthroughLayerMask);
			SetGrounded ();
			return delta;
		}
		...
		virtual public void Kill(DamageInfo info)
		{
			...
			// Hard reset swim when we die
			SpecialMovement_Swim swimMovementForReset = GetComponentInChildren<SpecialMovement_Swim>();
			if (swimMovementForReset != null) 
			{
				swimMovementForReset.StopSwim();
			}
		...
		/// </summary>
		virtual public void FinishedAttack(string name)
		{
			OnAttackFinished (name);
			activeAttack = -1;
		}
		...
		virtual public void HitEnemy(IMob enemy, DamageInfo info)
		{
			...
			if (activeMovement != -1 && movements[activeMovement] is BasicAttacks) ((BasicAttacks)movements[activeMovement]).HitEnemy(enemy, info);
			OnAttackCausedDamage (enemy, info);
		}
		...
		virtual protected void Init()
		{
			...
			if (input == null)
			{
				...
				if (inputs.Length == 1) 
				{
					...
					{
						input = inputs[0];
						if (input.playerId != PlayerId && input.playerId != -1) Debug.LogWarning ("Inputs player id does not match this characters player id.");
					}
				}
				...
				else 
				{
					...
					{
						if (potentialInput is BufferedInput && (potentialInput.playerId == PlayerId || potentialInput.playerId == -1))
						{
                    ...
                    if (input == null) foreach (Input potentialInput in inputs)
					{

						if (potentialInput is MultiInput && (potentialInput.playerId == PlayerId || potentialInput.playerId == -1))
						{
                            input = potentialInput;
							break;
						}
                    ...
                    if (input == null) foreach (Input potentialInput in inputs)
					{
						if (potentialInput.playerId == PlayerId || potentialInput.playerId == -1)
						{
							input = potentialInput;
							break;
						}
			...
			damageEventArgs = new ExtraDamageInfoEventArgs ();
            ...
            Breath = GetComponentInChildren<Breath>();
            if (CharacterHealth != null) CharacterHealth.Init (this);
			if (ItemManager != null) ItemManager.Init (this);
			if (PowerUpManager != null) PowerUpManager.Init (this);
			if (Inventory != null) Inventory.Init (this);
			if (EquipmentManager != null) EquipmentManager.Init (this);
		}
		...
		virtual protected void TransitionMovementControl()
		{
			...
			for (int i = 0; i < movements.Length; i++)
			{
				if (movements[i].enabled && movements[i].CheckAdditionalConditions() && movements[i] is BasicAttacks && 
			    ...
			    )
				{
					...
					if (newActiveAttack == -1 && movements[i] is BasicAttacks && ((BasicAttacks)movements[i]).WantsAttack())
					{
						...
						if (wantsControl) {
#if UNITY_EDITOR
							if (activeMovement != i && movements[activeMovement].ForceMaintainControl()) 
							{
// 								Debug.LogWarning("Attacks wants control at the same time as a movement, this may lead to unpredictable behaviour. Consider using an attack that does not override movement state.");
							...
							movements[i].ConditionsActivated();
							OnAttackStarted();
							return;
						}
						else
			...
			if (movements[activeMovement].enabled && movements[activeMovement].ForceMaintainControl()) return;
			...
			for (int i = 0; i < movements.Length; i++)
			{
				if (movements[i].enabled && movements[i].CheckAdditionalConditions() && 
				    // Some platforms might disallow (skip) certain types of movements, lets check this
				 	...
				 	(StoodOnPlatform == null || StoodOnPlatform == parentPlatform || !StoodOnPlatform.SkipMovement(this, movements[i])))
				{
					...
					// Ladders
					if (movements[i] is ClimbMovement && (activeAttack == -1 || !((BasicAttacks)movements[activeAttack]).BlockClimb()) && currentLadder != null)
					{
					...
					// Walls
					if (movements[i] is WallMovement && (activeAttack == -1 || !((BasicAttacks)movements[activeAttack]).BlockWall()))
					{
					...
					if (movements[i] is AirMovement && (activeAttack == -1 || !((BasicAttacks)movements[activeAttack]).BlockJump()) && ((AirMovement)movements[i]).WantsJump())
					{
						if (activeMovement != i)
						{
							movements[activeMovement].LosingControl();
					...
					else if (movements[i] is AirMovement && ((AirMovement)movements[i]).WantsAirControl())
					{
						if (activeMovement != i)
						{
							// For air movemnts we don't call conditions.Activated if they were activated only if they want jump
							movements[activeMovement].LosingControl();
							movements[i].GainControl();
						}
						activeMovement = i;
					...
					if (movements[i] is GroundMovement && ((GroundMovement)movements[i]).WantsGroundControl())
					{
						if (activeMovement != i)
						{
							movements[activeMovement].LosingControl();
			...
			if (grounded && Velocity.y <= 0)
			{
				...
				activeMovement = defaultGroundMovement;
				// Let each condition know it was activated
				movements[activeMovement].ConditionsActivated();
			}
			// Airborne
			else
			{
		...
		/// </summary>
		virtual public void UpdateCurrentWall()
		{
			int colliderCount = 0;
			...
			if (defaultWallMovement != -1)
			{
				for (int i = 0; i < CurrentCollisions.Length; i++) {
					if (CurrentCollisions [i] != null && 
						((Colliders [i].RaycastType == RaycastType.SIDE_LEFT && LastFacedDirection == -1) || 
							(Colliders [i].RaycastType == RaycastType.SIDE_RIGHT && LastFacedDirection == 1))) {
						for (int j = 0; j < CurrentCollisions [i].Length; j++) {
							if (CurrentCollisions [i] [j].collider != null && CurrentCollisions [i] [j].collider.gameObject != null &&
				...
				if (tempWall == null || (colliderCount < minimumWallColliders))
				{
					colliderCount = 0;
					tempWall = null;
					for (int i = 0; i < CurrentCollisions.Length; i++) {
				...
				if (tempWall != null && colliderCount >= minimumWallColliders)
				{
					CurrentWallColliderCount = colliderCount;
		...
		virtual protected void UpdateClosestColliders(RaycastType typeMask)
		{
			...
			for (int i = 0; i < CurrentCollisions.Length; i++)
			{
				...
				if ((Colliders[i].RaycastType & typeMask) == Colliders[i].RaycastType)
				{
                    ...
                    if (Colliders[i].RaycastType == RaycastType.HEAD && closest != -1 
                        && ((1 << CurrentCollisions[i][closest].collider.gameObject.layer & geometryLayerMask) == 1 << CurrentCollisions[i][closest].collider.gameObject.layer))
                    {
                        WouldHitHeadThisFrame = true;
                    }
				}
			}
		}
