...
namespace PlatformerPro.Tween
{
	...
	public class PositionTweener : MonoBehaviour, ITweener <Transform, Vector3>
	{
        ...
        virtual public void TweenWithTime (TweenMode tweenMode, Transform target, Vector3 destination, float time, Action<Transform, Vector3> callback)
		{
			if (Active) Stop ();
			this.time = time;
			this.target = target;
        ...
        /// <returns>The isntance doing the tween.</returns>
        virtual public void TweenWithRate (TweenMode tweenMode, Transform target, Vector3 destination, float speed, Action<Transform, Vector3> callback)
		{
			if (Active)Stop ();
			this.target = target;
			this.destination = destination;
