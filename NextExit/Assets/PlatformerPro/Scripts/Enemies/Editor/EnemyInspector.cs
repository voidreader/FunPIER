...
namespace PlatformerPro
{
	...
	public class EnemyInspector : PersistableObjectInspector
	{
		/// <summary>
		...
		protected SerializedProperty enemyPersistenceImplementation;
		...
		override protected void OnEnable()
		{
			...
			enemyPersistenceImplementation = serializedObject.FindProperty("enemyPersistenceImplementation");
		...
		/// </summary>
		override protected void DrawHeader(PlatformerProMonoBehaviour myTarget)
		{
			sceneViewInspectorId = this.GetInstanceID ();
			base.DrawHeader (myTarget);
		}
		...
		override protected void DrawInspector()
		{
			...
			if (!useDefaultPersistence.boolValue)
			{
				...
				{
					EditorGUILayout.PropertyField (enemyPersistenceImplementation);
		...
		virtual protected void ShowEditFeet(Enemy enemy)
		{
			...
			Undo.RecordObject (enemy, "Enemy Feet Update");
		...
		{
			Undo.RecordObject (enemy, "Enemy Side Update");
		...
		virtual protected void AddHazard()
		{
			...
			if (layer <= 0) 
			{
				layer = LayerMask.NameToLayer ("Enemy_Hazard");
				if (layer <= 0) 
				{
