...
namespace PlatformerPro
{
	...
	public class EnemyMovement_MeleeAttack : EnemyMovement, ICompletableMovement
	{
		...
		virtual protected IEnumerator DoAttack()
		{
			...
			// Face enemy
			if (faceEnemyOnAttack && enemy.CurrentTarget != null)
			{
				if (enemy.CurrentTargetTransform.position.x > transform.position.x) facingDirection = 1;
				else if (enemy.CurrentTargetTransform.position.x < transform.position.x) facingDirection = -1;
