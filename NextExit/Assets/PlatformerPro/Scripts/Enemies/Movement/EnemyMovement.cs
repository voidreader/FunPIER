...
namespace PlatformerPro
{
	...
	public abstract class EnemyMovement : MonoBehaviour
	{
		...
		/// </summary>
		/// <returns></returns>
		virtual public bool CheckForEdge(int movementDirection)
		{
			if (enemy.State == EnemyState.DEAD) return false;
			if (!enemy.characterCanFall)
			{
				Debug.LogWarning("Can't check for edges on a character that can't fall!");
				return false;
			}

			if (movementDirection == 1 && !enemy.GroundedRight) return true;
			if (movementDirection == -1 && !enemy.GroundedLeft) return true;
			if (!enemy.Grounded) return true;
			return false;
		}
