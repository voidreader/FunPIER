...
{
	/// <summary>
	/// Inspector for rail platforms.
	/// </summary>
	[CustomEditor(typeof(RailPlatform), true)]
	public class RailPlatformInspector : Editor
	{
		/// <summary>
		...
		public override void OnInspectorGUI()
		{
			...
			// Ensure 1 waypoint
			if (((RailPlatform)target).waypoints == null || ((RailPlatform)target).waypoints.Count < 2)
			{
				List<Vector2> waypoints = new List<Vector2>();
				waypoints.Add (((RailPlatform)target).transform.position);
				waypoints.Add (((RailPlatform)target).transform.position + Vector3.one);
				((RailPlatform)target).waypoints = waypoints;
			}
		...
		{			
			...
			if (sceneViewInspectorId != this.GetInstanceID ()) 
			{
				...
				RailPlatformInspector c = (RailPlatformInspector) EditorUtility.InstanceIDToObject(sceneViewInspectorId);
				if (c != null) 
				{
					c.OnSceneGUI();
					return;
			...
			ShowWaypoints ((RailPlatform)target);
			if (!Application.isPlaying)
			{
				
				if (editWaypoints)
				{
					ShowEditWaypoints(((RailPlatform)target));
				}
			}
		}
