...
namespace PlatformerPro
{
	...
	public class SpawnOnHeadbutt : Platform
	{
		...
		{
			isInNudge = true;
			if (spawnsRemaining > 0) spawnsRemaining--;
		...
		{
			// base.ApplyCustomPersistence (data);
			if (data.extraStateInfo != null)
			{
				int.TryParse (data.extraStateInfo, out spawnsRemaining);
				...
				if (randomItemSpawner != null)
				{
					randomItemSpawner.numberToSpawn = spawnsRemaining;
				}
			}
		}
		...
		override protected string ExtraPersistenceData
		{
			get
			{
				return spawnsRemaining.ToString();
			}
		}
