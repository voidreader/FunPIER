	...
	public class BasicAttacks : Movement
	{
		...
		virtual public void HitEnemy(IMob enemy, DamageInfo info)
		{
            ...
            if (currentAttack != -1 && !attacks[currentAttack].loseDurabilityOnUse &&
                (attacks[currentAttack].durabilitySlot != null &&
                 attacks[currentAttack].durabilitySlot != "" && 
                 attacks[currentAttack].durabilitySlot != "NONE"))
            {
                character.ItemManager.DamageItemInEquipmentSlot(attacks[currentAttack].durabilitySlot, 1);
            }
        }
		...
		virtual protected IEnumerator DoAttack(int attackIndex)
		{
            ...
            if (attacks[attackIndex].loseDurabilityOnUse && 
                (attacks[attackIndex].durabilitySlot != null && attacks[attackIndex].durabilitySlot != "" && attacks[attackIndex].durabilitySlot != "NONE"))
            {
                character.ItemManager.DamageItemInEquipmentSlot(attacks[attackIndex].durabilitySlot, 1);
            }
            // Set cooldown
		...
		virtual public void InstantiateProjectile(int attackIndex)
		{
			...
			if (projectile != null) {
				...
				projectile.Fire (attacks[attackIndex].damageAmount, attacks[attackIndex].damageType, direction, character, Charge);
			}
