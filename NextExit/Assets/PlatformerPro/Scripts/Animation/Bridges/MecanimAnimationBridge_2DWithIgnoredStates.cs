...
namespace PlatformerPro
{
    ...
    /// This variation only plays animations you specifically add to a list.
	/// </summary>
	public class MecanimAnimationBridge_2DWithIgnoredStates : PlatformerProMonoBehaviour, IAnimationBridge
	{
		
		#region members

        /// <summary>
        /// Only animation states in this list will be played.
        /// </summary>
        [Tooltip("Only animation states in this list will be played.")]
        public List<AnimationState> animationsToPlay;

        /// <summary>
        /// The animation played when the state being sent by the character isn't in the animationsToPlay list.
        /// </summary>
        [Tooltip("The animation played when the state being sent by the character isn't in the animationsToPlay list.")]
        public AnimationState defaultState;

        /// <summary>
        /// Maps from states to animator overrides.
        /// </summary>
        [Tooltip("Maps from states to animator overrides.")]
        ...
        override public string Header
        {
            get
            {
                return "This animator is typically used to implement weapons and equipment. It only plays animations you specifically " +
                	"add to the animationsToPlay list and is mainly meant as a time saver. Note that it wont work in more complex " +
                    " cases so you may need to use a standard animator and add animations for each state (even if its the using the" +
                    "same animation for different states).";
            }
        }
		...
		{
            // Get matching states or otherwise set to default state
            AnimationState filteredState = defaultState;
            for (int i = 0; i < animationsToPlay.Count; i++)
            {
                if (animationsToPlay[i] == args.State)
                {
                    filteredState = args.State;
                    break;
                }
            }
            ...
            if (myAnimator.runtimeAnimatorController is UnityEditor.Animations.AnimatorController)
			{
				if (!editor_stateNames.Contains(filteredState.AsString()))
				{
					Debug.LogError("The state " + filteredState.AsString() + " is not in the animator, state names must be an exact UPPERCASE match for the Animation State. This is being skipped in editor mode but this will lead to errors in release builds.");
					return;
				}
			}
#else
            ...
            if (queuedStates.Count < 1 || queuedStates.Peek() != filteredState)
			{
				queuedStates.Enqueue (filteredState);
