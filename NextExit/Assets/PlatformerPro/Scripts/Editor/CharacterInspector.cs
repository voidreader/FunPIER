...
namespace PlatformerPro
{
	...
	public class CharacterInspector : Editor
	{
        ...
        /// </summary>
        protected static bool validateSettings = true;
		...
		protected SerializedProperty minFastFallPassthroughTime;
		
		// Slope properties
		protected SerializedProperty calculateSlopes;
		protected SerializedProperty upSlopeDeceleration;
		protected SerializedProperty downSlopeAcceleration;
		...
		void OnEnable()
		{
			...
			minFastFallPassthroughTime = serializedObject.FindProperty("minFastFallPassthroughTime");
			
			calculateSlopes = serializedObject.FindProperty("calculateSlopes");
			upSlopeDeceleration = serializedObject.FindProperty("upSlopeDeceleration");
			downSlopeAcceleration = serializedObject.FindProperty("downSlopeAcceleration");
		...
		protected virtual void EnsureInitialised()
		{
			...
			// Ensure all colliders initialised properly (sometimes data gets corrupted when there are editor errors)
			for (int i = 0; i < feetColliders.Count; i++)
			{
				if (feetColliders[i].Transform == null)
				{
					feetColliders[i] = CreateDefaultCollider(RaycastType.FOOT, feetColliders[i].Extent);
				}
			}
			for (int i = 0; i < headColliders.Count; i++)
			{
				if (headColliders[i].Transform == null)
				{
					headColliders[i] = CreateDefaultCollider(RaycastType.HEAD, headColliders[i].Extent);
				}
			}
			for (int i = 0; i < leftColliders.Count; i++)
			{
				if (leftColliders[i].Transform == null)
				{
					leftColliders[i] = CreateDefaultCollider(RaycastType.SIDE_LEFT, leftColliders[i].Extent);
				}
			}
			for (int i = 0; i < rightColliders.Count; i++)
			{
				if (rightColliders[i].Transform == null)
				{
					rightColliders[i] = CreateDefaultCollider(RaycastType.SIDE_RIGHT, rightColliders[i].Extent);
				}
			}
		}
		...
		public override void OnInspectorGUI()
		{
			...
			feetColliders = ((Character)target).feetColliders.ToList();
			leftColliders = ((Character)target).leftColliders.ToList();
			rightColliders = ((Character)target).rightColliders.ToList();
			headColliders = ((Character)target).headColliders.ToList();
            ...
            DrawMajorWarnings((Character)target);
        }
		...
		virtual protected void DrawSlopesFoldout()
		{
			...
			if(slopesFoldOut)
			{
				...
				{
					EditorGUILayout.PropertyField(upSlopeDeceleration, new GUIContent("Up Slope Deceleration", "Effect of walking up a slope on base max speed. 0 = none, 1 = reduce speed by sine of angle."));
					EditorGUILayout.PropertyField(downSlopeAcceleration, new GUIContent("Down Slope Accleration", "Effect of walking down a slope on base max speed. 0 = none, 1 = increase speed by sine of angle."));
					if (upSlopeDeceleration.floatValue != 0 || downSlopeAcceleration.floatValue != 0) {
						EditorGUILayout.HelpBox("Note: Technically these parameters affect the max speed, the acceleration is part of the movement code. Most typically you want a positive value for the up value (e.g. 1), and a negative value for the down value (e.g -0.5).", MessageType.Info);
					}
		...
		virtual protected void DrawAvancedSettingsFoldout()
		{
			...
			if(advancedFoldOut)
			{
				...
				if (GUILayout.Button(new GUIContent("Reset To Defaults","Resets all advanced settings to default values.")))
				{
					...
					minFastFallPassthroughTime.floatValue = Character.DefaultMinFastFallPassthroughTime;
				...
				EditorGUILayout.PropertyField(minFastFallPassthroughTime, new GUIContent("Passthrough Fast Fall", "How long do we need to be falling before we consider passthroughs that we have fallen past due to our speed. Bigger characters might need higher values to avoid snapping."));
		...
		{
			Undo.RecordObject(target, "Collider Update");
			EnsureTwoFeetColliders();
		...
		virtual protected Vector3 DrawMoveHandle(BasicRaycast collider)
		{
			...
			guiPos = new Vector3(guiPos.x / EditorGUIUtility.pixelsPerPoint, guiPos.y / EditorGUIUtility.pixelsPerPoint, guiPos.z);
#endif
			// Adjusment Handles
			...
			Vector3 result = Handles.FreeMoveHandle(
				...
				Vector3.one, Handles.SphereHandleCap) - collider.Transform.position;
        ...
        /// </summary>
        virtual protected void DrawMajorWarnings(Character target)
        {
            GUILayout.Box("", GUILayout.ExpandWidth(true), GUILayout.Height(1));
            GUILayout.Label("Configuration Validation");
            if (validateSettings)
            { 
                long a =System.DateTime.Now.Millisecond;
                List<ValidationResult> currentResults = new List<ValidationResult>();
                currentResults.AddRange(new ValidateBasicMovements().Validate(target));
                currentResults.AddRange(new ValidateRaycastColliders().Validate(target));
                currentResults.AddRange(new ValidateLayers().Validate(target));
                currentResults.AddRange(new ValidateRigidbodies().Validate(target));
                currentResults.AddRange(new ValidateCharacterSettings().Validate(target));
            ...
            GUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            GUI.color = validateSettings ? Color.red : Color.white;
            if (GUILayout.Button((validateSettings ? "Hide" : "Show"), EditorStyles.miniButton))
            {
