	...
	public class MultiInput : Input
	{
		...
		void Start()
		{
			if (inputs == null || inputs.Length == 0) {
                List<Input> potentialInputs = new List<Input>();
                foreach (Input i in gameObject.GetComponentsInChildren<Input>()) 
                {
                    if (!(i is MultiInput))
                    {
                ...
                inputs = potentialInputs.ToArray();
			}
            if (inputs == null || inputs.Length == 0)
            {
                Debug.LogError("No inputs supplied to MultiInput, you should define some!");
            }
		}
		...
		override public void ForceButtonState (int index, ButtonState state)
		{
			for (int i = 0; i < inputs.Length; i++)
			{
				inputs[i].ForceButtonState(index, state);
			}
		}
