...
namespace PlatformerPro
{
	...
	public class StandardInput : Input
	{
		...
		/// </summary>
		protected ButtonState[] forceButtonState;
		...
		override public string Header
		{
			get
			{
				return "The standard input used for non-mobile devices. Supports keyboards, joysticks " +
					   "and controllers, and supports configuring input at runtime.";
			}
		}
		...
		void LateUpdate() 
		{
			...
			if (forceButtonState != null)
			{
				for (int i = 0 ; i < forceButtonState.Length; i++)
				{
					forceButtonState[i] = ButtonState.NONE;
				}
			}
		}
			...
			if (data.Length > 0)
			{
				try 
				{
					...
					using (StringReader stream = new StringReader(data)) {
						...
						// Update force button state
						forceButtonState = new ButtonState[actionButtons.Length];
						for (int i = 0 ; i < forceButtonState.Length; i++)
						{
							forceButtonState[i] = ButtonState.NONE;
						}
		...
		public void SetActionButton(int index, ButtonState state)
		{
		...
		{
			if (buttonIndex == -1) return ButtonState.NONE;
			if (buttonIndex < actionButtons.Length)
			{
				if (forceButtonState[buttonIndex] != ButtonState.NONE)
				{
					return forceButtonState[buttonIndex];
				}
				return GetStateForKey(actionButtons[buttonIndex]);
			}
			return ButtonState.NONE;
		}
		...
		{
			if (index >= 0 && index < forceButtonState.Length) forceButtonState [index] = state;
		}
