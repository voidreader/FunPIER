...
namespace PlatformerPro
{
	...
	public class BasicTouchInput : Input
	{
        ...
        /// </summary>
        protected int cachedVerticalAxisDigital = UNSET;

        /// <summary>
        /// Cached state of the horizontal axis (digital).
        /// </summary>
        protected int cachedHorizontalAxisDigital = UNSET;
        ...
        /// <summary>
        /// If a cached value is this it hasn't yet been used
        /// </summary>
		...
		void LateUpdate()
		{
            ...
            if (cachedHorizontalAxisDigital != UNSET)
            {
                lastDigitalHorizontalAxisState = cachedHorizontalAxisDigital;
            }
            else
            {
                lastDigitalHorizontalAxisState = HorizontalAxisDigital;
            }
            if (cachedVerticalAxisDigital != UNSET)
            {
                lastDigitalVerticalAxisState = cachedVerticalAxisDigital;
            }
            else
            {
                lastDigitalVerticalAxisState = HorizontalAxisDigital;
            }
            cachedHorizontalAxisDigital = UNSET;
            cachedVerticalAxisDigital = UNSET;
		...
		override public int HorizontalAxisDigital
		{
			get
			{
                if (cachedHorizontalAxisDigital != UNSET) return cachedHorizontalAxisDigital;
                if (actualHorizontalAxis == null) return 0;
                // Get horizontal axis value
                float result = actualHorizontalAxis.Value;
                // Right
                if (result > 0)
                {
                    cachedHorizontalAxisDigital = 1; 
                    return 1;
                }
                // Left
                if (result < 0)
		...
		override public int VerticalAxisDigital
		{
			get
			{
                if (cachedVerticalAxisDigital != UNSET) return cachedVerticalAxisDigital;
                if (actualVerticalAxis == null ) return 0;
				// Get horizontal axis value
		...
		override public void ForceButtonState (int index, ButtonState state)
		{
			Debug.LogWarning ("Touch input does not support force button state");
		}
