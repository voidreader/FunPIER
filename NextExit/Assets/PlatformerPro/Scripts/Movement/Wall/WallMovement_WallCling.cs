...
namespace PlatformerPro
{
	...
	public class WallMovement_WallCling : WallMovement
	{
		...
		/// </summary>
		protected bool clingStarted;

		/// <summary>
		/// Cached wall direction.
		/// </summary>
		protected int currentWallDirection;
		...
		/// </summary>
		private const string Description = "Wall movement which allows the character to stick to and slide down certain walls." +
		...
		/// </summary>
		protected const float DefaultIgnoreJumpTime = 0.2f;

		/// <summary>
		/// The default ignore cling time.
		/// </summary>
		protected const float DefaultIgnoreClingTime = 0.5f;
		...
		override public AnimationState AnimationState
		{
			get 
			{
				if (hasJumped)
				{
					return AnimationState.WALL_JUMP;
				}
				if (ignoreGravityTimer > 0.0f) return AnimationState.WALL_CLING;
...
//			if (hit.fraction > 0) character.Translate (hit.point.x - character.Colliders [character.CurrentWallCollider].WorldExtent.x, 0, false);
		...
		public static MovementVariable[] DrawInspector(MovementVariable[] movementData, ref bool showDetails, Character target) {
			return DrawInspectorWithOptions (movementData, ref showDetails, target, true, true);
		}

		/// <summary>
		/// Draws the inspector.
		/// </summary>
		public static MovementVariable[] DrawInspectorWithOptions(MovementVariable[] movementData, ref bool showDetails, Character target, bool showGravity, bool showAutoStick)
		{
			if (movementData == null || movementData.Length < MovementVariableCount)
			{
			...
			if (showDetails)
			{
				...
				if (movementData[IgnoreClingAfterJumpTimeIndex] == null) movementData[IgnoreClingAfterJumpTimeIndex] = new MovementVariable(DefaultIgnoreClingTime);
				...
				// Ignore jump time
				if (movementData[IgnoreJumpTimeIndex] == null) movementData[IgnoreJumpTimeIndex] = new MovementVariable(DefaultIgnoreJumpTime);
				...
				// Must hold towards wall constantly to maintain cling
