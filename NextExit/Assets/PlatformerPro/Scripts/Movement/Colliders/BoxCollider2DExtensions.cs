...
namespace PlatformerPro
{
	...
	public static class BoxCollider2DExtensions
	{
        ...
        public static Vector2[] GetXExtentsInWorldSpace(this BoxCollider2D box)
        {
            Vector2 min;
            Vector2 max;
            min = (Vector2)box.transform.TransformPoint(box.Offset() + new Vector2((-box.size.x / 2.0f), 0));
            max = (Vector2)box.transform.TransformPoint(box.Offset() + new Vector2((box.size.x / 2.0f), 0));
            return new Vector2[] { min, max };
        }
