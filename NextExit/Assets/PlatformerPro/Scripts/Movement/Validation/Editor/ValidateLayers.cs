...
#else
				result.Add (new ValidationResult("The maximum layer is higher than " + EXPECTED_MAX_LAYER + " we recommend only " + (EXPECTED_MAX_LAYER + 1) + " layers for performance reasons.", MessageType.Info));
#endif
			}
			return result;
		}
		...
		protected List<ValidationResult> CheckGeometryLayers(Character c)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				if ((c.layerMask & (1 << i)) == (1 << i))
				{
					...
					{
						result.Add (new ValidationResult("For performance reasons the default raycasts wont check layers higher than " + (NoAllocationRaycast.MAX_LAYER - 1) + ". Contact support if you need this to change.", MessageType.Warning));
					}
				}
			}
