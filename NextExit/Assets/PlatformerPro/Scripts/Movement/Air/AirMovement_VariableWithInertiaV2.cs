...
namespace PlatformerPro
{
	...
	/// </summary>
	public class AirMovement_VariableWithInertiaV2 : AirMovement_Variable
	{
		...
		/// </summary>
		protected bool isTravellingFasterThanMax;
		...
		/// Human readable name.
		/// </summary>
		private const string Name = "Physics/Variable Height Jump v2";
		...
		private const string Description = "An air movement that has a variable height jump and in which the horizontal speed " +
						...
						"version separates acceleration and drag";
		...
		/// The index of the air speed acceleration in the movement settings.
		/// </summary>
		protected const int AirSpeedDecelerationIndex = 9;
		...
		{
			isForceRun = false;
			isTravellingFasterThanMax = false;
			// Force a run if the previous movement was a run
			if (character.ActiveMovement is GroundMovement_DigitalDoubleTapRun) isForceRun = true;
			// Force a walk if we aren't close to run speed
			if (Mathf.Abs(character.Velocity.x) > airSpeed) isTravellingFasterThanMax = true;
		}
		...
		override protected void MoveInX (float horizontalAxis, int horizontalAxisDigital, ButtonState runButton)
		{
			...
			{
				// Faster than air speed then decelerate
				if (character.Velocity.x > airSpeed)
				{
					character.AddVelocity(-airSpeedDeceleration * TimeManager.FrameTime, 0, false);
				} 
				// Slower than air speed then accelerate
				else if (character.Velocity.x < airSpeed)
				{
			...
			if (isTravellingFasterThanMax && Mathf.Abs(character.Velocity.x) <= airSpeed) isTravellingFasterThanMax = false;
