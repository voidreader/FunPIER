...
namespace PlatformerPro
{
	...
	public class DamageMovement_AnimationWithBobble : DamageMovement
	{
		...
		override public void DoMove()
		{
			...
			if (character.Velocity.y <= 0) hasReachedPeak = true;
		...
		override public bool ForceMaintainControl()
		{
			...
			// If we have reached peak of velocity and something stops us falling return control
			if (hasAppliedVelocity && hasReachedPeak && character.Velocity.y >= 0) 
			{
				return false;
			}
			return true;
		...
		override public RaycastType ShouldDoBaseCollisions
		{
			get
			{
				if (!isDeath && hasReachedPeak) return RaycastType.FOOT;
