...
namespace PlatformerPro
{
	...
	public class GroundMovement_PhysicsWithRun: GroundMovement_Physics
	{
		...
		override public void DoMove()
		{
			...
			float actualWalkSpeed = GetSpeed (walkSpeed);
			float actualMaxSpeed = GetRunSpeed (maxSpeed);
			float actualAcceleration = GetAcceleration (acceleration);
			...
			// Apply acceleration 
			if (Mathf.Abs (character.Input.HorizontalAxisDigital * actualAcceleration) > ignoreForce )
			{
				...
				{
					if (Mathf.Abs (character.Velocity.x) < actualWalkSpeed)
					{
						character.AddVelocity((float)character.Input.HorizontalAxisDigital * actualAcceleration * TimeManager.FrameTime, 0, false);
						hasAppliedForce = true;
						// Limit to walk speed
						if (character.Velocity.x > actualWalkSpeed) 
						{
							character.SetVelocityX(actualWalkSpeed);
						}
						else if (character.Velocity.x < -actualWalkSpeed) 
						{
							character.SetVelocityX(-actualWalkSpeed);
						}
					}
			...
			// Limit to max speed
			if (character.Velocity.x > actualMaxSpeed) 
			{
				character.SetVelocityX(actualMaxSpeed);
			}
			else if (character.Velocity.x < -actualMaxSpeed) 
			{
				character.SetVelocityX(-actualMaxSpeed);
			}
