...
namespace PlatformerPro
{
	...
	public class GroundMovement_CrouchWithSlide : GroundMovement_Crouch
	{
		...
		override public void DoMove()
		{
            ...
            // Apply drag
            if (character.SlopeTargetRotation == 0 
                && (!character.WouldHitHeadThisFrame || Mathf.Abs(character.Velocity.x) > (maxSpeed / 2.0f)))
            {
            ...
            // Accelerate or deccelerate if on a slope
            if (slopeAccelerationFactor != 0)
            {
                if (character.SlopeTargetRotation != 0)
                {
                    character.AddVelocity(TimeManager.FrameTime * slopeAccelerationFactor * -character.Gravity * Mathf.Sin(character.SlopeTargetRotation * Mathf.Deg2Rad), 0, true);
                }
            }
