...
namespace PlatformerPro
{
	...
	public class GroundMovement_RollOnLand : GroundMovement
	{
		...
		{
			float actualSpeed = GetSpeed (speed);
			if (rollTimer > 0.0f)
			{
				if (character.Input.HorizontalAxisDigital == 0 || character.Input.HorizontalAxisDigital == currentRollDirection)
				{
					character.SetVelocityX(actualSpeed * currentRollDirection);
					character.Translate(actualSpeed * currentRollDirection * TimeManager.FrameTime, 0, false);
				}
			}
		}
