...
{
	[CustomEditor(typeof(PlatformerProRuleTile))]
	[CanEditMultipleObjects]
	internal class PlatformerProRuleTileEditor : Editor
	{
		...
		public PlatformerProRuleTile tile { get { return (target as PlatformerProRuleTile); } }
		...
		public void OnEnable()
		{
			if (tile.m_TilingRules == null)
				tile.m_TilingRules = new List<PlatformerProRuleTile.TilingRule>();

			m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(PlatformerProRuleTile.TilingRule), true, true, true, true);
		...
		private float GetElementHeight(int index)
		{
			if (tile.m_TilingRules != null && tile.m_TilingRules.Count > 0)
			{
				...
				{
					case PlatformerProRuleTile.TilingRule.OutputSprite.Random:
						return k_DefaultElementHeight + k_SingleLineHeight*(tile.m_TilingRules[index].m_Sprites.Length + 3 + + tile.m_TilingRules[index].m_supportingData.Length) + k_PaddingBetweenRules;
					case PlatformerProRuleTile.TilingRule.OutputSprite.Animation:
						return k_DefaultElementHeight + k_SingleLineHeight*(tile.m_TilingRules[index].m_Sprites.Length + 2 + tile.m_TilingRules[index].m_supportingData.Length) + k_PaddingBetweenRules;
				}
			}
			return k_DefaultElementHeight + k_SingleLineHeight + k_SingleLineHeight*(tile.m_TilingRules[index].m_supportingData.Length) + k_PaddingBetweenRules;
		}

		private void OnDrawElement(Rect rect, int index, bool isactive, bool isfocused)
		{
			PlatformerProRuleTile.TilingRule rule = tile.m_TilingRules[index];
		...
		private static void RuleMatrixOnGUI(Rect rect, PlatformerProRuleTile.TilingRule tilingRule)
		{
			...
			for (int y = 0; y <= 2; y++)
			{
				for (int x = 0; x <= 2; x++)
				{
					...
					if (x != 1 || y != 1)
					{
						...
						{
							case PlatformerProRuleTile.TilingRule.Neighbor.This:
								GUI.DrawTexture(r, arrows[y*3 + x]);
								break;
							case PlatformerProRuleTile.TilingRule.Neighbor.NotThis:
								GUI.DrawTexture(r, arrows[9]);
								break;
						}
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_Neighbors[index] = (PlatformerProRuleTile.TilingRule.Neighbor) (((int)tilingRule.m_Neighbors[index] + 1) % 3);
							GUI.changed = true;
							Event.current.Use();
						}
					...
					else
					{
						...
						{
							case PlatformerProRuleTile.TilingRule.Transform.Rotated:
								GUI.DrawTexture(r, autoTransforms[0]);
								break;
							case PlatformerProRuleTile.TilingRule.Transform.MirrorX:
								GUI.DrawTexture(r, autoTransforms[1]);
								break;
							case PlatformerProRuleTile.TilingRule.Transform.MirrorY:
						...
						if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
						{
							tilingRule.m_RuleTransform = (PlatformerProRuleTile.TilingRule.Transform)(((int)tilingRule.m_RuleTransform + 1) % 4);
							GUI.changed = true;
							Event.current.Use();
						}
					}
		...
		{
			public PlatformerProRuleTile.TilingRule m_Rule;
			public PlatformerProRuleTile.TilingRule.Transform m_NewValue;

			public MenuItemData(PlatformerProRuleTile.TilingRule mRule, PlatformerProRuleTile.TilingRule.Transform mNewValue)
			{
				this.m_Rule = mRule;
		...
		private void SpriteOnGUI(Rect rect, PlatformerProRuleTile.TilingRule tilingRule)
		{
		...
		private static void RuleInspectorOnGUI(Rect rect, PlatformerProRuleTile.TilingRule tilingRule)
		{
			float y = rect.yMin;
			EditorGUI.BeginChangeCheck();
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
			tilingRule.m_RuleTransform = (PlatformerProRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RuleTransform);
			...
			tilingRule.m_Output = (PlatformerProRuleTile.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;
			
			if (tilingRule.m_Output == PlatformerProRuleTile.TilingRule.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == PlatformerProRuleTile.TilingRule.OutputSprite.Random)
			{
				...
				tilingRule.m_RandomTransform = (PlatformerProRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
				y += k_SingleLineHeight;
			}

			if (tilingRule.m_Output != PlatformerProRuleTile.TilingRule.OutputSprite.Single)
			{
			...
			GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Type");
			tilingRule.m_TileType = (PlatformerProRuleTile.TilingRule.TileType)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width  - k_LabelWidth, k_SingleLineHeight), tilingRule.m_TileType);
			y += k_SingleLineHeight;
			
			switch (tilingRule.m_TileType) {
				case PlatformerProRuleTile.TilingRule.TileType.Generic_Prefab:
					
					if (tilingRule.m_supportingData.Length != 1)
					{
						tilingRule.m_supportingData = new MovementVariable[1];
						tilingRule.m_supportingData[0] = new MovementVariable() { GameObjectValue = null};
					}
					GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Prefab");
					tilingRule.m_supportingData[0].GameObjectValue = (GameObject)EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width  - k_LabelWidth, k_SingleLineHeight), (Object)tilingRule.m_supportingData[0].GameObjectValue, typeof(GameObject), true);
					y += k_SingleLineHeight;  
					break;
                case PlatformerProRuleTile.TilingRule.TileType.Ladder:
                    if (tilingRule.m_supportingData.Length != 1)
                    {
                        tilingRule.m_supportingData = new MovementVariable[1];
                        tilingRule.m_supportingData[0] = new MovementVariable() { GameObjectValue = null };
                    ...
                    y += k_SingleLineHeight;
                    break;
                case PlatformerProRuleTile.TilingRule.TileType.Hazard:
					
					if (tilingRule.m_supportingData.Length != 3)
					{
						tilingRule.m_supportingData = new MovementVariable[3];
						tilingRule.m_supportingData[0] = new MovementVariable() { GameObjectValue = null};
						tilingRule.m_supportingData[1] = new MovementVariable() { IntValue = 0};
						tilingRule.m_supportingData[2] = new MovementVariable() { IntValue = 1};
					}
					GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Prefab");
					tilingRule.m_supportingData[0].GameObjectValue = (GameObject)EditorGUI.ObjectField(new Rect(rect.xMin + k_LabelWidth, y, rect.width  - k_LabelWidth, k_SingleLineHeight), (Object)tilingRule.m_supportingData[0].GameObjectValue, typeof(GameObject), true);
					y += k_SingleLineHeight;
					GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Damage Amount");
					tilingRule.m_supportingData[1].IntValue = EditorGUI.IntField(new Rect(rect.xMin + k_LabelWidth, y, rect.width  - k_LabelWidth, k_SingleLineHeight), tilingRule.m_supportingData[1].IntValue);
					y += k_SingleLineHeight;
					GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Damage Type");
					tilingRule.m_supportingData[2].IntValue = (int)(DamageType)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width  - k_LabelWidth, k_SingleLineHeight), (DamageType)tilingRule.m_supportingData[2].IntValue);
					y += k_SingleLineHeight;
					break;
				default:
					if (tilingRule.m_supportingData.Length != 0)
					{
						tilingRule.m_supportingData = new MovementVariable[0];
					}
					break;
			}
		}
