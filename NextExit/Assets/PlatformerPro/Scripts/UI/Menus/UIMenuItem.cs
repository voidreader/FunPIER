	...
	public class UIMenuItem : MonoBehaviour
	{
		...
		virtual public void DoAction()
		{
			...
			case UIMenuAction.LOAD_SCENE:
                ...
                { 
                    LevelManager.Instance.LoadScene (supportingString);
                }
                break;
			...
			case UIMenuAction.USE_ITEM_AND_HIDE:
				...
				break;
			case UIMenuAction.NUMBER_OF_PLAYERS:
				Debug.LogWarning("You must use UIMenuItem_NumberOfPlayers when using NUMBER_OF_PLAYERS action type.");
				break;
			case UIMenuAction.ACTIVATE_ITEM:
				ag = gameObject.GetComponent<ActivationGroup>();
				if (ag != null) ag.Activate(supportingString);
				else Debug.LogWarning("Could not ACTIVATE_ITEM because no activation group could be found.");
				break;
			case UIMenuAction.DEACTIVATE_ITEM:
				ag = gameObject.GetComponent<ActivationGroup>();
				if (ag != null) ag.Deactivate(supportingString);
				else Debug.LogWarning("Could not DEACTIVATE_ITEM because no activation group could be found.");
				break;
			default:
#if UNITY_EDITOR
