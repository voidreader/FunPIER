...
//			str = str.Replace("\\", "");
			...
			if(str.Length > 0) {
                ...
                else if (str[0] == '"' || str[0] == System.Convert.ToChar("'"))
                {
					type = Type.STRING;
					...
					try {
						...
						for(int i = 1; i < str.Length; i++) {
                            ...
                            if (str[i] == '"' || str[i] == System.Convert.ToChar("'"))
								openquote = !openquote;
							if(str[i] == '[' || str[i] == '{')
								depth++;
	...
	public void Add(string obj) {
		Add (new JSONObject { type = JSONObject.Type.STRING, str = obj });
	}
	...
	public JSONObject GetField(string name) {
		...
		Debug.Log( "(string)keys[i] : " + (string)keys[0] );
	...
	public string print(bool isPretty=false) {
        return print(0, isPretty);
	}
    public string print(int depth, bool isPretty = false)
    ...
    public Dictionary<string, object> ToDictionary()
    {
        ...
        if (type == Type.OBJECT)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                switch (val.type)
                {
                    case Type.OBJECT: result.Add((string)keys[i], val.ToDictionary()); break;
                    case Type.ARRAY: result.Add((string)keys[i], val.ToArray()); break;
                    case Type.STRING: result.Add((string)keys[i], val.str); break;
                    //case Type.NUMBER:	result.Add((string)keys[i], val.n + "");	break;
                    case Type.NUMBER: result.Add((string)keys[i], val.str); break;
                    case Type.BOOL: result.Add((string)keys[i], val.b + ""); break;
                    case Type.NULL:
                        result.Add((string)keys[i], null);
                        break;
    ...
    public ArrayList ToArray()
    {
        ...
        if (type == Type.ARRAY)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                switch (val.type)
                {
                    case Type.OBJECT: result.Add(val.ToDictionary()); break;
                    case Type.ARRAY: result.Add(val.ToArray()); break;
                    case Type.STRING: result.Add(val.str); break;
                    //case Type.NUMBER: result.Add(val.n + ""); break;
                    case Type.NUMBER: result.Add(val.str); break;
                    case Type.BOOL: result.Add(val.b + ""); break;
                    case Type.NULL: result.Add(null); break;
                    default: Debug.LogWarning("Omitting object: in array conversion"); break;
                }
            }
            // 결과가 1개밖에 없고 null이면 빈 ArrayList를 리턴한다.
            if (result.Count == 1 && result[0] == null)
                return new ArrayList();
            return result;
        }
        else Debug.LogWarning("Tried to turn non-Object JSONObject into a array");
        return null;
    }
}
