...
namespace SA.Foundation.Editor
{
    public abstract class SA_PluginSettingsWindow<TWindow> : EditorWindow where TWindow : EditorWindow
    {
        ...
        [SerializeField]
        SA_HyperLabel m_documentationLink;

        //MenuTabs
        [SerializeField]
        protected bool m_isToolBarWasAlreadyCreated;
        [SerializeField]
        protected SA_HyperToolbar m_menuToolbar;
        [SerializeField]
        protected List<SA_GUILayoutElement> m_tabsLayout = new List<SA_GUILayoutElement>();
        ...
        {
            if (Event.current.type == EventType.Repaint) m_headerHeight = GUILayoutUtility.GetLastRect().yMax + SA_PluginSettingsWindowStyles.LAYOUT_PADDING;
            ...
            using (new SA_GuiBeginScrollView(ref m_scrollPos, GUILayout.Width(position.width), GUILayout.Height(position.height - m_headerHeight)))
            {
                ...
                GUILayout.Space(1);
                if (Event.current.type == EventType.Repaint) m_scrollContentHeight = GUILayoutUtility.GetLastRect().yMax + SA_PluginSettingsWindowStyles.LAYOUT_PADDING;
