...
namespace SA.Foundation.Editor
{
    public class SA_PluginAboutLayout : SA_GUILayoutElement
    {
        //What we do
        [SerializeField]
        SA_HyperLabel m_games;
        [SerializeField]
        SA_HyperLabel m_plugins;
        [SerializeField]
        SA_HyperLabel m_team;

        //How to get in touch
        [SerializeField]
        SA_HyperLabel m_linkedIn;
        [SerializeField]
        SA_HyperLabel m_twitter;
        [SerializeField]
        SA_HyperLabel m_facebook;

        [SerializeField]
        SA_HyperLabel m_youtube;
        [SerializeField]
        SA_HyperLabel m_google;
        [SerializeField]
        SA_HyperLabel m_twitch;

        [SerializeField]
        SA_HyperLabel m_support_mail;
        [SerializeField]
        SA_HyperLabel m_ceo_mail;

        [SerializeField]
        SA_HyperLabel web_siteLabel;

        const int LABEL_WIDTH = 100;
        const int SOCIAL_LABEL_WIDTH = 70;
        ...
        public override void OnLayoutEnable()
        {
            ...
            m_support_mail = CreateTextLabel(SA_Config.StansAssetsSupportEmail);
            m_ceo_mail = CreateTextLabel(SA_Config.StansAssetsCeoEMail);
        ...
        public override void OnGUI()
        {
            ...
            using (new SA_WindowBlockWithIndent(new GUIContent("What we do")))
            {
                ...
                {
                    var games = m_games.Draw(GUILayout.Width(LABEL_WIDTH));
                    if (games) Application.OpenURL("https://stansassets.com/#portfolio");

                    var plugins = m_plugins.Draw(GUILayout.Width(LABEL_WIDTH));
                ...
                using (new SA_GuiBeginHorizontal())
                {
                    var team = m_team.Draw(GUILayout.Width(LABEL_WIDTH));
            ...
            using (new SA_WindowBlockWithIndent(new GUIContent("How to get in touch")))
            {
                ...
                using (new SA_GuiBeginHorizontal())
                {
                    if (m_support_mail.Draw()) Application.OpenURL("mailto:" + SA_Config.StansAssetsSupportEmail);
                }
                ...
                using (new SA_GuiBeginHorizontal())
                {
                    ...
                    if (clicked) Application.OpenURL("mailto:" + SA_Config.StansAssetsCeoEMail);
                }
                ...
                using (new SA_GuiBeginHorizontal())
                {
                    ...
                    if (clicked) Application.OpenURL(SA_Config.StansAssetsWebsiteRootUrl);
                }
            }
        }

        SA_HyperLabel CreateAboutLabel(string title, string icon)
        {
            return CreateLabel(title, icon, SA_Skin.k_AboutIconsPath);
        }

        SA_HyperLabel CreateSocialLabel(string title, string icon)
        {
            return CreateLabel(title, icon, SA_Skin.k_SocialIconsPath);
        }

        SA_HyperLabel CreateLabel(string title, string icon, string iconFolder)
        {
        ...
        SA_HyperLabel CreateTextLabel(string title)
        {
