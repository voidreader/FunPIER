...
{
    public static class AMM_Manager
    {
        static AMM_Template s_template = null;

        public static bool ManifestExists => SA_AssetDatabase.IsFileExists(AMM_Settings.MANIFEST_FILE_PATH);

        static void ReadManifest(string manifestPath)
        {
#if !(UNITY_WP8 || UNITY_METRO)
            ...
            foreach (XmlAttribute attr in rootNode.Attributes) s_template.SetValue(attr.Name, attr.Value);
            ...
            foreach (XmlAttribute attr in applicationNode.Attributes) s_template.ApplicationTemplate.SetValue(attr.Name, attr.Value);
            ...
            foreach (XmlNode childNode in applicationNode.ChildNodes)
                    ...
                    && !childNode.Name.Equals("#comment"))
                {
                    ...
                    var activity = new AMM_ActivityTemplate(isLauncher, activityName);
                    foreach (XmlAttribute attr in childNode.Attributes) activity.SetValue(attr.Name, attr.Value);
            ...
            foreach (XmlNode node in rootNode.ChildNodes)
                ...
                {
                    var permission = new AMM_PropertyTemplate("uses-permission");
        ...
        public static void SaveManifest()
        {
            ...
            if (!SA_AssetDatabase.IsFileExists(AMM_Settings.MANIFEST_FILE_PATH)) //Make sure we have a folder
            ...
            var child = newDoc.CreateElement("manifest");
            ...
            var pattern = @"android___";
            var replacement = "android:";
        ...
        {
            var property = new AMM_PropertyTemplate(node.Name);

            //Get Values
            foreach (XmlAttribute attr in node.Attributes) property.SetValue(attr.Name, attr.Value);
