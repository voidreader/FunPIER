...
namespace SA.Android.Manifest
{
    ...
    {
        readonly Dictionary<int, AMM_ActivityTemplate> _activities = null;

        public AMM_ApplicationTemplate()
            : base()
        {
        ...
        {
            var activity = GetActivityWithName(name);
            if (activity == null)
            {
        ...
        public override void ToXmlElement(XmlDocument doc, XmlElement parent)
        {
            ...
            foreach (var id in _activities.Keys)
            {
        ...
        public Dictionary<int, AMM_ActivityTemplate> Activities => _activities;
    }
}
