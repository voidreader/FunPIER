...
namespace SA.Android.Manifest
{
    ...
    public class AMM_AndroidManifest
    {
        ...
        AMM_Template m_template;

        public AMM_Template Template => m_template;
        ...
        void ReadManifest(string manifestPath)
        {
            m_template = new AMM_Template();

            if (!SA_AssetDatabase.IsFileExists(manifestPath)) return;
            ...
            foreach (XmlAttribute attr in rootNode.Attributes) m_template.SetValue(attr.Name, attr.Value);
            ...
            foreach (XmlAttribute attr in applicationNode.Attributes) m_template.ApplicationTemplate.SetValue(attr.Name, attr.Value);
        ...
        public void SaveManifest()
        {
            ...
            {
                var m_folderPath = SA_PathUtil.GetDirectoryPath(m_path);
                if (!SA_AssetDatabase.IsValidFolder(m_folderPath)) SA_AssetDatabase.CreateFolder(m_folderPath);
            }
