...
- (void)productsRequest:(nonnull SKProductsRequest *)request didReceiveResponse:(nonnull SKProductsResponse *)response {
    ...
    SA_PluginSettingsWindowStylesitResult *result =  [[SA_PluginSettingsWindowStylesitResult alloc] initWithSKProductsRespons:response];
    ISN_SendMessage(UNITY_SK_LISTENER, "OnStoreKitDidReceiveResponse", [result toJSONString]);
    [[NSNotificationCenter defaultCenter] postNotificationName:kInAppPurchaseManagerProductsFetchedNotification object:self userInfo:nil];
}

- (void)request:(SKRequest *)request didFailWithError:(NSError *)error {
    SA_PluginSettingsWindowStylesitResult* result = [[SA_PluginSettingsWindowStylesitResult alloc] initWithNSError:error];
    ISN_SendMessage(UNITY_SK_LISTENER, "OnStoreKitDidReceiveResponse", [result toJSONString]);
}
