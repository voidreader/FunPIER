...
namespace HT
{
	...
	public class BezierCurve : MonoBehaviour
	{
			...
			get
			{
				if (dirty)
				{
					...
					for (int i = 0; i < points.Length - 1; i++)
						_length += ApproximateLength(i, resolution);

					if (close)
						_length += ApproximateLength(points.Length - 1, resolution);
		...
		private float[] approximateLengths = null;
		...
		private void Update()
		{
			...
			Vector3 vScale = transform.lossyScale;
		...
		/// </param>
		public Vector3 GetPointAt(float t, bool lerpByCurvePercent)
		{
			...
			if (lerpByCurvePercent)
			{
				...
				for (int i = 0; i < points.Length - 1; i++)
				{
					curvePercent = ApproximateLength(i) / length;
			...
			{
				int nPointCount = (_close) ? points.Length : points.Length - 1;
				float fRatePerPoint = 1.0f / nPointCount;
				float fTimeRate = t / fRatePerPoint;

				int nFirstIndex = Mathf.FloorToInt(fTimeRate);
				p1 = points[nFirstIndex];

				if (nFirstIndex + 1 < points.Length)
					p2 = points[nFirstIndex + 1];
				else
					p2 = points[0];

				return GetPoint(p1, p2, (t - (nFirstIndex * fRatePerPoint)) / fRatePerPoint);
			}
		}
		...
		public float ApproximateLength(int nIndex, int resolution = 10)
		{
			if (approximateLengths == null)
				approximateLengths = new float[points.Length];

			if (dirty || approximateLengths[nIndex] <= 0.0f)
			{
				int nNextIndex = nIndex + 1;
				if (nNextIndex >= points.Length)
					nNextIndex = 0;

				approximateLengths[nIndex] = ApproximateLength(points[nIndex], points[nNextIndex], resolution);
			}

			return approximateLengths[nIndex];
		}
