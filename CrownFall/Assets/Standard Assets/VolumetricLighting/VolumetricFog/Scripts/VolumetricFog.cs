...
public class VolumetricFog : MonoBehaviour
{
	...
	RenderTexture _blankedRenderTexture = null;
	...
	void SetUpPointLightBuffers(int kernel)
	{
		...
		if (count == 0)
		{
			if (m_PointLightParamsCB == null)
				m_PointLightParamsCB = new ComputeBuffer(1, 32);

			m_PointLightParamsCB.SetCounterValue(0);
	...
	void SetUpTubeLightBuffers(int kernel)
	{
		...
		if (count == 0)
		{
			if (m_TubeLightParamsCB == null)
				m_TubeLightParamsCB = new ComputeBuffer(1, 48);

			m_TubeLightParamsCB.SetCounterValue(0);
			m_InjectLightingAndDensity.SetBuffer(kernel, "_TubeLights", m_TubeLightParamsCB);

			//-----
			if (m_TubeLightShadowPlaneParamsCB == null)
				m_TubeLightShadowPlaneParamsCB = new ComputeBuffer(1, 48);

			m_TubeLightShadowPlaneParamsCB.SetCounterValue(0);
	...
	void SetUpAreaLightBuffers(int kernel)
	{
		...
		if (count == 0)
		{
			if (m_AreaLightParamsCB == null)
				m_AreaLightParamsCB = new ComputeBuffer(1, 96);

			m_AreaLightParamsCB.SetCounterValue(0);
			m_InjectLightingAndDensity.SetBuffer(kernel, "_AreaLights", m_AreaLightParamsCB);

			//-----
			if (_blankedRenderTexture == null)
				_blankedRenderTexture = new RenderTexture(1, 1, 16, RenderTextureFormat.ARGB32);

			m_InjectLightingAndDensity.SetTexture(kernel, "_AreaLightShadowmap", _blankedRenderTexture);
			return;
		}
	...
	void SetUpFogEllipsoidBuffers(int kernel)
	{
		...
		if (count == 0)
		{
			if (m_FogEllipsoidParamsCB == null)
				m_FogEllipsoidParamsCB = new ComputeBuffer(1, 64);

			m_FogEllipsoidParamsCB.SetCounterValue(0);
