...
namespace VolumetricFogAndMist {
	...
	public class VolumetricFogProfile: ScriptableObject {
		...
		[SerializeField, Range(0, 1f)] public float lightScatteringDiffusion = 0.7f;
		[SerializeField, Range(0, 1f)] public float lightScatteringSpread = 0.686f;
		[SerializeField,Range(4, 64)] public int lightScatteringSamples = 16;
		[SerializeField,Range(0, 50f)] public float lightScatteringWeight = 1.9f;
		[SerializeField,Range(0, 50f)] public float lightScatteringIllumination = 18f;
		[SerializeField,Range(0.9f, 1.1f)] public float lightScatteringDecay = 0.986f;
		[SerializeField,Range(0, 0.2f)] public float lightScatteringExposure = 0;
		[SerializeField,Range(0, 1f)] public float lightScatteringJittering = 0.5f;
		...
		public static void Lerp(VolumetricFogProfile profile1, VolumetricFogProfile profile2, float t, VolumetricFog fog) {
			...
			if (profile1.fogVoidOverride && profile2.fogVoidOverride) {
				fog.fogVoidDepth = profile1.fogVoidDepth * (1f - t) + profile2.fogVoidDepth;
				fog.fogVoidFallOff = profile1.fogVoidFallOff * (1f - t) + profile2.fogVoidFallOff;
				fog.fogVoidHeight = profile1.fogVoidHeight * (1f - t) + profile2.fogVoidHeight;
				fog.fogVoidPosition = profile1.fogVoidPosition * (1f - t) + profile2.fogVoidPosition;
				fog.fogVoidRadius = profile1.fogVoidRadius * (1f - t) + profile2.fogVoidRadius;
			}

			// Light Scattering
			if (profile1.lightScatteringOverride && profile2.lightScatteringOverride) {
				fog.lightScatteringDecay = profile1.lightScatteringDecay * (1f - t) + profile2.lightScatteringDecay;
				fog.lightScatteringDiffusion = profile1.lightScatteringDiffusion * (1f - t) + profile2.lightScatteringDiffusion;
				fog.lightScatteringExposure = profile1.lightScatteringExposure * (1f - t) + profile2.lightScatteringExposure;
				fog.lightScatteringIllumination = profile1.lightScatteringIllumination * (1f - t) + profile2.lightScatteringIllumination;
				fog.lightScatteringJittering = profile1.lightScatteringJittering * (1f - t) + profile2.lightScatteringJittering;
				fog.lightScatteringSamples = (int)(profile1.lightScatteringSamples * (1f - t) + profile2.lightScatteringSamples);
				fog.lightScatteringSpread = profile1.lightScatteringSpread * (1f - t) + profile2.lightScatteringSpread;
				fog.lightScatteringWeight = profile1.lightScatteringWeight * (1f - t) + profile2.lightScatteringWeight;
			}
		}
