...
public class SteamManager : MonoBehaviour
{
	...
	{
		HT.HTDebug.PrintLog(HT.eMessageType.Warning, pchDebugText.ToString());
	}
	...
	private void Awake()
	{
		...
		if (!Packsize.Test())
		{
			HT.HTDebug.PrintLog(HT.eMessageType.Error, "[Steamworks.NET] Packsize Test returned false, the wrong version of Steamworks.NET is being run in this platform.", this);
		}

		if (!DllCheck.Test())
		{
			HT.HTDebug.PrintLog(HT.eMessageType.Error, "[Steamworks.NET] DllCheck Test returned false, One or more of the Steamworks binaries seems to be the wrong version.", this);
		}
		...
		try
		{
			...
			if (SteamAPI.RestartAppIfNecessary(new AppId_t(GameDefine.nSteamID)))
			{
				Application.Quit();
				return;
			}
		...
		{ // We catch this exception here, as it will be the first occurence of it.
			HT.HTDebug.PrintLog(HT.eMessageType.Error, "[Steamworks.NET] Could not load [lib]steam_api.dll/so/dylib. It's likely not in the correct location. Refer to the README for more details.\n" + e, this);
		...
		if (!m_bInitialized)
		{
			HT.HTDebug.PrintLog(HT.eMessageType.Error, "[Steamworks.NET] SteamAPI_Init() failed. Refer to Valve's documentation or the comment above this line for more information.", this);
