    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				#if TWISTUV_ON
				fixed2 tempUv = i.uv - fixed2(_TwistUvPosX *  _MainTex_ST.x, _TwistUvPosY *  _MainTex_ST.x);
				...
				tempUv += fixed2(_TwistUvPosX *  _MainTex_ST.x, _TwistUvPosY *  _MainTex_ST.x);
				i.uv = tempUv;
				#endif
				...
				#if TEXTURESCROLL_ON
				i.uv.x += _Time.y * _TextureScrollXSpeed;
				i.uv.y += _Time.y * _TextureScrollYSpeed;
				#endif
				...
				#if DISTORT_ON
				i.uvDistTex.x += _Time * _DistortTexXSpeed;
				i.uvDistTex.y += _Time * _DistortTexYSpeed;
				...
				fixed angWave = sqrt(dot(uvWave, uvWave)) * _WaveAmount - (_Time.y *  _WaveSpeed);
				i.uv = i.uv + normalize(uvWave) * sin(angWave) * (_WaveStrength / 1000);
				#endif

				#if WIND_ON
				#if !MANUALWIND_ON
				i.uv.x = fmod(abs(lerp(i.uv.x, i.uv.x + (_GrassWind * 0.01 * sin(_Time * _GrassSpeed * 10)), i.uv.y)), 1);
				#else
				i.uv.x = fmod(abs(lerp(i.uv.x, i.uv.x + (_GrassWind * 0.01 * _GrassManualAnim), i.uv.y)), 1);
				#endif
				#endif
				...
				#if BLUR_ON
				#if !BLURISHD_ON
				col.rgb = BlurHD(i.uv, _MainTex, _BlurIntensity) * col.a;
				#else
				col.rgb = Blur(i.uv, _MainTex, _BlurIntensity) * col.a;
				#endif
				#endif
				...
				col.rgb += tex2D(_MainTex, i.uv + rot(fixed2(-_MotionBlurDist, -_MotionBlurDist))) * col.a;
				col.rgb += tex2D(_MainTex, i.uv + rot(fixed2(-_MotionBlurDist * 2, -_MotionBlurDist * 2))) * col.a;
				col.rgb += tex2D(_MainTex, i.uv + rot(fixed2(-_MotionBlurDist * 3, -_MotionBlurDist * 3))) * col.a;
				col.rgb += tex2D(_MainTex, i.uv + rot(fixed2(-_MotionBlurDist * 4, -_MotionBlurDist * 4))) * col.a;
				...
				#if GRADIENT_ON
				fixed2 tiledUvGrad = fixed2(i.uv.x / _MainTex_ST.x, i.uv.y / _MainTex_ST.y);
				...
				//OUTLINE-------------------------------------------------------------
					...
					#if OUTDIST_ON
					i.uvOutDistTex.x += _Time * _OutlineDistortTexXSpeed;
					i.uvOutDistTex.y += _Time * _OutlineDistortTexYSpeed;
					...
					#if OUTTEX_ON
					i.uvOutTex.x += _Time * _OutlineTexXSpeed;
					i.uvOutTex.y += _Time * _OutlineTexYSpeed;
