...
fixed4 Blur(fixed2 uv, sampler2D source, fixed Intensity)
{
	...
	texCoord = uv + fixed2(-step, -step); result += tex2D(source, texCoord);
	texCoord = uv + fixed2(-step, 0); result += 2.0 * tex2D(source, texCoord);
	texCoord = uv + fixed2(-step, step); result += tex2D(source, texCoord);
	texCoord = uv + fixed2(0, -step); result += 2.0 * tex2D(source, texCoord);
